...
import SlidersFormPictures from "./SlidersFormPictures.vue";
...
const createCategory = (values, actions) => {
    axios.post('/api/sliders/create', form)
        .then((response) => {
...
const editCategory = (values, actions) => {
    axios.put(`/api/sliders/${route.params.id}/edit`, form)
        .then((response) => {
            router.push('/admin/sliders');
...
const getCategory = () => {
    axios.get(`/api/sliders/${route.params.id}/edit`)
        .then(({ data }) => {
            console.log(data);
            form.id = data.id;
            form.name = data.name;
...
const handleFileChange = async (event) => {
    ...
    axios.post('/api/sliders/upload-image', formData)
        .then((response) => {
...
<template>
            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item">
                            <router-link to="/admin/sliders">Slider</router-link>
                        </li>
    ...
    <sliders-form-pictures></sliders-form-pictures>
</template>
