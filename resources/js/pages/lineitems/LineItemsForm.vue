...
const createCategory = (values, actions) => {
    axios.post('/api/lineitems/create', form)
        .then((response) => {
            router.push('/admin/lineitems');
...
const editCategory = (values, actions) => {
    axios.put(`/api/lineitems/${route.params.id}/edit`, form)
        .then((response) => {
            router.push('/admin/lineitems');
...
const getCategory = () => {
    axios.get(`/api/lineitems/${route.params.id}/edit`)
        .then(({ data }) => {
            console.log(data);
            form.id = data.id;
            form.name = data.name;
...
onMounted(() => {
    if (route.name === 'admin.lineitems.edit') {
        editMode.value = true;
        getCategory();
    }
});
...
const handleFileChange = async (event) => {
    ...
    axios.post('/api/lineitems/upload-image', formData)
        .then((response) => {
            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item">
                            <router-link to="/admin/lineitems">Service</router-link>
                        </li>
