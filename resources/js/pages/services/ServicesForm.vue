...
import ServicesFormPictures from "./ServicesFormPictures.vue";
...
const createCategory = (values, actions) => {
    axios.post('/api/services/create', form)
        .then((response) => {
...
const editCategory = (values, actions) => {
    axios.put(`/api/services/${route.params.id}/edit`, form)
        .then((response) => {
            router.push('/admin/services');
...
const getCategory = () => {
    axios.get(`/api/services/${route.params.id}/edit`)
        .then(({ data }) => {
            console.log(data);
            form.id = data.id;
            form.name = data.name;
...
onMounted(() => {
    if (route.name === 'admin.services.edit') {
        editMode.value = true;
        getCategory();
    }
});
...
const handleFileChange = async (event) => {
    ...
    axios.post('/api/services/upload-image', formData)
        .then((response) => {
...
<template>
            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item">
                            <router-link to="/admin/services">Service</router-link>
                        </li>
    ...
    <services-form-pictures></services-form-pictures>
</template>
